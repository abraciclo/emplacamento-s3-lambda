...
# Configuração Zabbix
# ============================
ZABBIX_SERVER="monitor2.blocktime.com.br"   # IP ou nome do servidor Zabbix
ZABBIX_HOSTNAME="162.ABRACICLO - EMPLACAMENTO LINUX"  # Nome do host cadastrado no Zabbix
S3_SUCCESS=0
S3_FAILURE=0
FTP_SUCCESS=0
FTP_FAILURE=0
...
echo "[INFO] Processando envio de arquivos..."
...
echo "[INFO] Enviando arquivos para $REMOTE_DIR (FTP + S3)"
...
for LOCAL_FILE in "$SOURCE_PATH"/*; do
  ...
  if [ "$FILE_SIZE" -eq 0 ]; then
    echo "[AVISO] Arquivo $FILE_NAME é zero byte, será ignorado."
    continue
  fi
...
EOF
  ...
  # ============================
  # Atualiza contadores Zabbix
  # ============================
  if [ $S3_BACKUP_STATUS -eq 0 ] && [ $S3_BMW_STATUS -eq 0 ] && [ $S3_HONDA_STATUS -eq 0 ] && \
     [ $S3_DN_STATUS -eq 0 ] && [ $S3_DT_STATUS -eq 0 ]; then
    ((S3_SUCCESS++))
  else
    ((S3_FAILURE++))
  fi
...
# ============================
# Envia métricas ao Zabbix (apenas se houve arquivos válidos)
# ============================
TOTAL_PROCESSED=$((S3_SUCCESS + S3_FAILURE + FTP_SUCCESS + FTP_FAILURE))
if [ "$TOTAL_PROCESSED" -gt 0 ]; then
  echo "[ZABBIX] Enviando métricas..."
  echo "[ZABBIX] S3_SUCCESS=$S3_SUCCESS, S3_FAILURE=$S3_FAILURE, FTP_SUCCESS=$FTP_SUCCESS, FTP_FAILURE=$FTP_FAILURE"

  zabbix_sender -z "$ZABBIX_SERVER" -s "$ZABBIX_HOSTNAME" -k upload.s3.success  -o "$S3_SUCCESS"
  zabbix_sender -z "$ZABBIX_SERVER" -s "$ZABBIX_HOSTNAME" -k upload.s3.failure  -o "$S3_FAILURE"
  zabbix_sender -z "$ZABBIX_SERVER" -s "$ZABBIX_HOSTNAME" -k upload.ftp.success -o "$FTP_SUCCESS"
  zabbix_sender -z "$ZABBIX_SERVER" -s "$ZABBIX_HOSTNAME" -k upload.ftp.failure -o "$FTP_FAILURE"
else
  echo "[INFO] Nenhum arquivo processado (todos ignorados ou inbox vazia). Nenhum dado enviado ao Zabbix."
fi

echo "[INFO] Execução finalizada."
